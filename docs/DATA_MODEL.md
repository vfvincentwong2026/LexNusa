...
> **文档版本**：v1.0.1 | **最后更新**：2026-08-27
> **v1.0.1 变更**：修正 SQL 硬伤——SQLite 不支持 `CREATE TABLE` 内联 `INDEX`，索引全部拆为独立 `CREATE INDEX` 语句；修正 §3.3 最短路径查询中 `json_each` 的非法用法；默认 Embedding 模型改为多语言 `bge-m3`。
...
CREATE TABLE IF NOT EXISTS edges (
  ...
  metadata      TEXT,                      -- 关系元数据（JSON 字符串，如 "生效日期"）
  created_at    TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
...
> **与 Pasal.id 关系类型的映射**（P0 实测确认，API `laws/{frbr_uri}` 返回）：
> `Mengubah → AMENDS`、`Mencabut → REPEALS`、`Merujuk → REFERENCES`、`Melaksanakan → IMPLEMENTS`（实施/下位执行，可映射为 DERIVES_FROM 的反向）、`Menguji → REVIEWS`（司法审查，原模型未覆盖，建议新增此类型）。
...
CREATE TABLE IF NOT EXISTS vector_meta (
  ...
  embedding_model   TEXT DEFAULT 'bge-m3', -- 多语言模型（中/印尼/英）
...
> ⚠️ 必须使用多语言 Embedding 模型 `@cf/baai/bge-m3`（Workers AI 目录已确认可用，2026-08-27）。英文模型（bge-*-en-v1.5）无法支撑中文/印尼语三语检索。
...
WITH RECURSIVE path_finder AS (
  ...
  WHERE pf.depth < 10
    -- 防环：json_each 是表值函数，必须用子查询形式
    AND NOT EXISTS (
      SELECT 1 FROM json_each(pf.path) je WHERE je.value = e.target_id
    )
)
SELECT * FROM path_finder WHERE target_id = 'PP_2021_9' ORDER BY depth LIMIT 1;
```
...
原方案"Fork indonesian-legal-network-analysis（Neo4j）"经 P0 核实不可行（该仓库无 LICENSE、无数据本体、2024-12 停更，详见 `docs/P0_DATA_FEASIBILITY.md`）。

**现行策略（方案 A）**：消费 Pasal.id API（`laws/{frbr_uri}` 返回条款全文 + 修订链关系），LexNusa 自建中文映射层与检索层。部层级法规（PMK/Permenaker/PER）缺口由第二数据源补齐（P1 任务）。

### 4.2 从 Pasal.id API 同步

1. 维护"核心法规清单 → frbr_uri"映射表（如 UU 25/2007 → `/akn/id/act/uu/2007/25`）。
2. 调用 `GET /api/v1/laws/{frbr_uri}` 获取条款全文 + relationships（修订链）。
3. 映射 Pasal.id 关系类型到 D1 `relation_type`（见 §2.2 映射表）。
4. 三重校验后入库：正文体积 + 关键词存在性 + 状态徽章（防伪 404 陷阱，见 P0 报告 §3.3）。
5. 增量更新（检测 `updated_at` 变化）。
...
以上索引已在 §2 各建表语句后随附 `CREATE INDEX` 语句，可直接执行。
