#!/usr/bin/env node
// 为 KUHPerdata Buku Ketiga 707 条款生成 bge-m3 向量（断点续跑）
const fs = require("fs");
...
const ZH = "《印尼民法典》（Burgerlijk Wetboek）";
...
(async () => {
  const arts = JSON.parse(fs.readFileSync("C:/Users/夏夜/AppData/Local/Temp/kuhper_b3_articles.json", "utf8"));
  const items = arts.map(a => ({
    id: `kuhperdata_pasal_${String(a.pasal).toLowerCase()}`,
    pasal: String(a.pasal),
    content: a.text,
  }));
  console.log("待向量化:", items.length);

  const outPath = "out/vectors_kuhper.ndjson";
  const done = new Set();
  ...
  const metaSql = fs.createWriteStream("out/vector_meta_kuhper.sql", { flags: "a" });
  for (let i = 0; i < todo.length; i += BATCH) {
    ...
    for (let k = 0; k < batch.length; k++) {
      const r = batch[k];
      out.write(JSON.stringify({ id: r.id, values: vectors[k], metadata: { law_id: "kuhperdata", pasal: r.pasal } }) + "\n");
