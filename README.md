...
**当前数据底座**：27 部核心法规（公司/劳工/税务/移民/土地/行业监管六大领域 + 印尼民法典第三编债法 707 条，为合同专家 Agent 铺路）· 4,011 条款（2,908 条有正文，其中 1,057 条由 peraturan.go.id 官方 PDF 补齐并标注来源）· 1,749 条修订/引用关系 · 2,908 条条款向量。数据源：[Pasal.id](https://pasal.id) API + 印尼官方 peraturan.go.id + 印尼维基文库（公共领域转录）。
...
- ☑ P2（进行中）：bge-m3 + Vectorize 语义检索上线；空条款补齐 1,057 条（peraturan.go.id 官方 PDF，含劳工法 Pasal 156、公司法 Pasal 142、工资条例/外籍劳工条例全量）；印尼民法典第三编债法 707 条入库（维基文库公共领域转录，无缺号校验通过）
- □ P2 收尾：商法典 KUHDagang 入库；剩余空条款（创造就业法 825 条需母法合并文本）；D3.js 图谱可视化；公司法专家 Agent
