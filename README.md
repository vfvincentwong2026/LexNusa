...
**当前数据底座**：28 部核心法规（公司/劳工/税务/移民/土地/行业监管六大领域 + 印尼民法典债法编 + 商法典，为合同/公司法专家 Agent 铺路）· 4,966 条款（3,863 条有正文，其中 1,057 条由 peraturan.go.id 官方 PDF 补齐并标注来源）· 1,749 条修订/引用关系 · 3,863 条条款向量。数据源：[Pasal.id](https://pasal.id) API + 印尼官方 peraturan.go.id + 印尼维基文库（公共领域转录）。
...
| **Agent 专家平台** | 五大专家 Agent：**公司法 / 税务 / 劳工 / 印尼合同专家 / 财务税务专家**（合同审查·风险识别·双语起草·财税实操），MCP 协议调用底层图谱 | ⏳ P2–P3 待做 |
...
- ☑ P2（进行中）：bge-m3 + Vectorize 语义检索上线；空条款补齐 1,057 条（peraturan.go.id 官方 PDF，含劳工法 Pasal 156、公司法 Pasal 142、工资条例/外籍劳工条例全量）；印尼民法典第三编债法 707 条 + 商法典 KUHDagang 955 条入库（维基文库公共领域转录，缺号校验通过，源文本固有缺号已在 metadata 标注）
- □ P2 收尾：剩余空条款（创造就业法 825 条需母法合并文本）；D3.js 图谱可视化；公司法专家 Agent
- □ 印尼合同专家 Agent：合同审查（风险条款识别 + 法条出处）+ 双语合同起草
- □ 财务税务专家 Agent：财税实操一体化——印尼会计准则（PSAK/SAK）记账与财报合规、报税日历与税款计算、转让定价文档、中印尼财税差异对照
